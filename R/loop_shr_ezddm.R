...
#' @export 结果
#'
loop_shr_ezddm <- function(list, Target, Paper_ID, Indice, nc) {
  ...
  for (i in 1:ceiling(list_length / subset_size)) {
    ...
    # 使用foreach循环并行执行迭代
    output <- foreach(j = 1:length(list_subset), .combine = rbind, .packages = c("dplyr", "tidyr", "hausekeep", "yukiBP")) %dopar% {

      # 调用函数并得到数据框
      result <- yukiBP::shr_ezddm(list = list_subset[[j]], Target = Target) %>%
